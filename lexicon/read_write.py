...
def fname_splitter(docslist):
    ...
    for doc in docslist:
        blocks = doc.split(
            '/')  # '\\' Windows, '/' Linux -  maybe this not relevant after refactoring of os.path/root.etc
...
def chain_ouput_file(chains, fname, outfolder):
    if (chains):  # only produce files with chains
        ...
        for chain in chains:
            doc_chain.write(chain.chain_id.iword + '\t' + str(chain.chain_id.isyn) + '\t' + str(
                chain.chain_id.ioffset) + '\t' + chain.chain_id.ipos + '\n')
        doc_chain.close()
    else:
